...
class Equipment(models.Model):
    ...
    # Поля для переопределения характеристик CarModel
    engine_volume = models.DecimalField(max_digits=3, decimal_places=1, blank=True, null=True, verbose_name="Объем двигателя (л)")
    power = models.PositiveIntegerField(blank=True, null=True, verbose_name="Мощность (л.с.)")
    transmission = models.CharField(max_length=20, choices=TRANSMISSION_CHOICES, blank=True, null=True, verbose_name="Коробка передач")
    acceleration = models.DecimalField(max_digits=3, decimal_places=1, blank=True, null=True, verbose_name="Разгон до 100 км/ч (сек)")
...
class ModelImage(models.Model):
    ...
    equipment = models.ForeignKey(Equipment, on_delete=models.SET_NULL, related_name='images', verbose_name="Комплектация", null=True, blank=True)
